...
No live server needed — `tests/conftest.py`'s `client` fixture drives the `PyWire` ASGI app in-process via `starlette.testclient.TestClient`. `tests/integration/test_login.py` is the reference example: plain-HTTP routes (`/login`, `/logout` — see Auth above) are just `client.post(...)`/`client.get(...)`, no WebSocket protocol needed.

**DB-backed tests need zero manual setup.** `tests/conftest.py`'s `postgres_url` fixture reuses `DATABASE_URL` if you've already set one (assumed migrated), otherwise auto-provisions a disposable Postgres via testcontainers and runs the real `alembic upgrade head` against it — no docker compose, no manual export required. If Docker isn't reachable it skips those tests instead of failing the run. `db_session_factory` layers on top of it, monkeypatching `database.engine`/`SessionLocal` so `database.get_session()` behaves exactly as it does in production for the duration of one test (see `TestLoginAgainstRealDatabase` in test_login.py for the pattern — same fixtures are meant to be reused by `tests/db/`'s real test cases once written).

A future WebSocket-driven test (for `@click`-heavy pages like contributions/withdrawals/medicalreceipts) would need a helper that speaks PyWire's msgpack event protocol directly via `client.websocket_connect("/_pywire/ws")` — not needed yet, since nothing currently under test uses it. `db`/`web`/`docker` categories are still skeletons pending real test cases — see each folder's `README.md`.
